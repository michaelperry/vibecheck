...
class AppStore: ObservableObject {
    ...
    func fetchGitHub() async {
        ...
        do {
                    ...
                    if createdAt >= todayStart && !isExcluded {
                        if commitMessages.isEmpty {
                            ...
                            for commit in commitMessages {
                                let fullMsg = commit["message"] as? String ?? ""
                                let firstLine = fullMsg.components(separatedBy: "\n").first ?? ""
                                let isAI = CommitItem.detectAIAssisted(fullMsg)
                                todayList.append(CommitItem(
                                    message: String(firstLine.prefix(72)),
            ...
            let humanCount = todayList.count - aiCount
            ...
            await MainActor.run {
                ...
                self.aiAssistedCommitsToday = aiCount
                self.humanCommitsToday = humanCount
...
struct CommitItem: Identifiable {
    ...
    static func detectAIAssisted(_ fullMessage: String) -> Bool {
        let lower = fullMessage.lowercased()
        let aiPatterns = [
        ...
        if fullMessage.contains("\u{1F916}") { return true }
        // Check co-authored-by lines for known AI email/name patterns
        let coAuthorPatterns = ["noreply@anthropic.com", "copilot", "cursor", "devin"]
        for line in fullMessage.lowercased().components(separatedBy: "\n") {
            if line.trimmingCharacters(in: .whitespaces).hasPrefix("co-authored-by:") {
                for p in coAuthorPatterns {
